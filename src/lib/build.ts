...
function entryPointsParts(pages: string | string[], suffixe=''){
    ...
    Pages.forEach( (p:string) => {
        ...
        EntryPoints = {...EntryPoints, [join(pagePath, pageName+suffixe)]: pageEntryPoint};
    })
    return { EntryPoints, pagesPaths, pagesNames}
}

async function pageApp(page: string | string[]){
    const {
        EntryPoints,
        //pagesPaths,
        //pagesNames
    } = entryPointsParts(page)
...
async function buildPage(pages: string | string[]){
    try{
        /* SSR
